...
/*ch32v307_spi_init函数 ch32v307_spi结构体中的SPI_HandleTypeDef型变量handle，并且完成SPI的初始化。函数*/
static rt_err_t ch32v307_spi_init(struct ch32v307_spi *spi_drv, struct rt_spi_configuration *cfg)
{
    ...
    /*配置SPI总线的数据方向：单线、双线*/
    if (cfg->mode & RT_SPI_3WIRE)  //此处有些不一样 但是寄存器的参数我选的一样，随后查看例程更改
    {
    ...
    ch32v307_spi_clock_and_io_init(spi_handle->Instance); //spi_drv->config->Instance 片上SPI
    ...
    /*此处为打印信息 -系统频率、pclk2频率、SPI限制的频率，波特率频率-由于SPI暂时没法设置波特率暂时未解决*/
                              ...
                              struct rt_spi_configuration *configuration)
{
    ...
    /*获取device->bus隶属的structure stm32_spi类型的结构体句柄*/
    /*然后更新结构体中的cfg */
...
/*始化和收发函数已经有了，把它们俩封装进struct rt_rt_spi_ops结构体里面*/
...
/**
  ...
  */
/* 注册好SPI总线之后，就可以往总线上挂载设备了，
 * 使用函数rt_hw_spi_device_attach() 将SPI设备挂载到SPI总线上，
 * 并且将SPI设备注册到IO设备管理器中*/
